...
def main():
    ...
    # Design comparison: an alternative direction, served as-is so it can be
    # opened side by side with the live site on a real device. Deliberately not
    # generated by page(): the whole point is that it shares no chrome.
    alt = os.path.join(ROOT, "preview", "instrument.html")
    if os.path.exists(alt):
        os.makedirs(os.path.join(OUT, "preview"), exist_ok=True)
        shutil.copy2(alt, os.path.join(OUT, "preview", "index.html"))
